...
export default function CoercivePanel() {
  const [monthly, setMonthly] = useState<GdeltCoerciveMonthly[]>([]);
  const [sources, setSources] = useState<GdeltCoerciveSource[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    Promise.all([loadGdeltCoerciveMonthly(), loadGdeltCoerciveSources()])
      .then(([m, s]) => { setMonthly(m); setSources(s); setLoading(false); })
      .catch((err) => { setError(err.message); setLoading(false); });
  }, []);

  if (loading) return <div className="loading-container"><div className="loading-spinner" /><span className="loading-text">Loading coercive discourse...</span></div>;
  ...
  return (
    <div>
      <h3>Coercive Discourse Analysis</h3>
      <p className="tab-subtitle">GDELT GKG coercive quotations — media rhetoric with threatening/coercive language</p>

      <div className="chart-card">
        <h3>Monthly Volume of Coercive Quotations</h3>
        <ResponsiveContainer width="100%" height={300}>
          <LineChart data={monthly} margin={{ top: 10, right: 20, bottom: 30, left: 20 }}>
            ...
            <Line type="monotone" dataKey="records" name="Coercive Records" stroke="#f97316" dot={false} strokeWidth={2} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      <div className="chart-card">
        <h3>Top 15 Media Sources</h3>
